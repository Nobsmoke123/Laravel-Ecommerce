								...
								<tbody>
									@foreach($products as $product)
										 <tr>
											...
											<td> <img class="img-responsive" src="{{ asset('../../'.$product->product_image)}}" alt="" width="300" height="100"> </td>
											 <td class="text-center"> {{ $product->name }} </td>
											<td class="text-center"> {{ $product->price }} </td>
											<td class="text-center"> {{ $product->discounted_price }} </td>
											<td class="text-center"> {{ $product->quantity }} </td>
											<td class="text-center"><a href="{{ route('admin:product-edit',['id'=>$product->id]) }}" class="btn btn-primary"><i class="fa fa-edit"></i></a></td>
											<td class="text-center"><a href="{{ route('admin:product-delete',['id'=>$product->id])}}" class="btn btn-danger" ><i class="fa fa-trash"></i></a></td> 
										</tr> 
									@endforeach
								</tbody>
							</table>
